...
- Health probes (`/actuator/health`, `/health/live`, `/health/ready`) answer WITHOUT credentials when pico-client-auth's middleware is active (structural `_pico_allow_anonymous` marker, no new dependency) — kubelet probes cannot send JWTs. `/info`, `/metrics` and `/refresh` remain subject to auth. Found by the flagship auth e2e.
